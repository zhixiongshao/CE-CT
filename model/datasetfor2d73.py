...
def train(model, train_loader, val_loader, num_epochs=100, lr=0.001):
    ...
    for epoch in range(num_epochs):
        ...
        train_loss /= total
        train_acc, train_precision, train_recall, train_f1, train_auc, train_specificity = calculate_metrics(train_labels, train_preds, train_probs)

        print(f"Train Loss: {train_loss:.4f} | Acc: {train_acc:.4f} | Precision: {train_precision:.4f} | Recall: {train_recall:.4f} | F1: {train_f1:.4f} | AUC: {train_auc:.4f} | Specificity: {train_specificity:.4f}")
        ...
        val_loss /= total
        val_acc, val_precision, val_recall, val_f1, val_auc, val_specificity = calculate_metrics(val_labels, val_preds, val_probs)

        print(f"Val Loss: {val_loss:.4f} | Acc: {val_acc:.4f} | Precision: {val_precision:.4f} | Recall: {val_recall:.4f} | F1: {val_f1:.4f} | AUC: {val_auc:.4f} | Specificity: {val_specificity:.4f}")
...
train_loader, val_loader = get_all_dataloaders(root_folder, batch_size=32)  # Test set merged into validation
